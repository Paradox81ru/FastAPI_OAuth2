...
- 1.1 [Асинхронная версия проекта.](#11-асинхронная-версия-проекта)
...
### 1.1 Асинхронная версия проекта.

Конечно, ***FastAPI*** из коробки является асинхронным, но в проекте также используется ***SQLAlchemy***. SQLAlchemy тоже
можно использовать в виде асинхронного кода, но в данном проекте код, относящийся к управлению базой данных посредством
библиотеки ***SQLAlchemy***, используется исключительно в синхронном виде. Это было сделано для более удобного использования ORM, который
в ***SQLAlchemy*** не весь предоставляет возможности асинхронности. Но, так же есть версия этого проекта полностью
переделанного в асинхронный вид, ознакомиться с ней можно в моём репозитории *github*, называется она 
[FastAPI_OAuth2_async](https://github.com/Paradox81ru/FastAPI_OAuth2_async).
