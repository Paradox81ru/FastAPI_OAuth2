...
для этого в адресной строке браузера следует ввести http://0.0.0.0:8001/docs. 

Либо можно открыть страницу авторизации, для этого следует ввести http://127.0.0.1:8000/index.
...
процедура происходит в автоматическом режиме, и не требует дополнительных действий от пользователя. Тоокне обновления тоже 
...
что использовался для формирования подписи (*HS256*), проверятся валидность подписи токена, и проверятся не истёк ли срок 
жизни токена. Если при проверке токена библиотека ***PyJWT*** не вернула исключение, значит предоставленный токен доступа 
валидный и действующий.