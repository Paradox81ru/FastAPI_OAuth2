...
class JWTTokenManager(BaseManager):
    ...
    def add_jwt_token(self, jti: UUID, data_expire: datetime, username: str):
    ...
    def get_jwt_token(self, jti: UUID | str):
    ...
    def get_user_jwt_token_count(self, username: str):
    ...
    def remove_jwt_token(self, jti: UUID | str):
    ...
    def remove_user_jwt_tokens(self, username: str):
        """ Удаляет все токены для указанного пользователя """
        # noinspection PyTypeChecker
        user: User = self._db.query(User).filter(User.username == username).one()
        user.jwt_tokens.clear()
        self._db.commit()

    def remove_expire_token(self):
